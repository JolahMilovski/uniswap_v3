...
use crate::{
    uniswap_graph::UniversalGraph, uniswap_v3::{process_pool_data, UniswapV3Pool}
};
            ...
            Ok(block_number) => {
                ...
                if n != last_sent_block {
                    ...
                    if let Err(e) = block_sender.send(n) {
                        error!("[UNISWAP_EVENTS_BLOCKS_ERROR] Ошибка отправки блока {}: {}", n, e);
                    } else {
                       // info!("[UNISWAP_EVENTS_BLOCKS] Отправлен новый блок: {}", n);
                    }
                } else {
//debug!("[UNISWAP_EVENTS_BLOCKS_DEBUG] Блок {} уже отправлен ранее, пропуск", n);
                }
            }
            Err(e) => {
                error!("[UNISWAP_EVENTS_BLOCKS_ERROR] Ошибка получения блока: {}", e);
            }
        }
    }
}
...
pub async fn fetch_events(
                ...
                let suggested_range = err_msg
                    ...
                    .and_then(|s| {
                        ...
                        if parts.len() == 2 {
                            ...
                            let to = u64::from_str_radix(parts[1].trim_end_matches(']').trim(), 16).ok()?;
                            Some((from, to))
                        } else {
                            None
                        }
                    });
                return Err(anyhow!("Ошибка RPC: {}. Предложенный диапазон: {:?}", err_msg, suggested_range));
            }
            return Ok(Vec::new());
        }
    };
    ...
    for log in logs {
        ...
        info!(
            "[UNISWAP_EVENTS_COUNTER_DEBUG] Обработано событий: event_id 🆔 {}",
            event_id + 1
        );
...
pub async fn fetch_tick_data(
    ...
    debug!("[    UNISWAP_EVENT_FETCH_TICK_DEBUG    ][{:?}] Текущий тик: {}",
    ...
    Ok(EventPoolUpdate {
     ...
     pub async fn update_graph_from_event(
        ...
        info!("[UNISWAP_EVENTS_UPDATE_GRAPH][{:?}] Завершено fetch_tick_data за {} мс",
            pool_address, fetch_duration.as_millis()
        );
        ...
        if let Some(pool) = current_graph.edges.get(&pool_address) {
            ...
            debug!("[UNISWAP_EVENTS_UPDATE_GRAPH_DEBUG][{:?}] Начало upsert_pool для пула, event_id: 🆔 {}", pool_address, pool_event_info.event_id);
            let _ = graph.load().upsert_pool(new_pool);
            let upsert_duration = upsert_start_time.elapsed();
            debug!("[UNISWAP_EVENTS_UPDATE_GRAPH_DEBUG][{:?}] Пул успешно обновлен в графе за {} мс, event_id: 🆔 {}", pool_address, upsert_duration.as_millis(), pool_event_info.event_id);
        } else {
            warn!(
...
fn aggregate_events(
    ...
    if !map.is_empty() {
        debug!(
            ...
            map.iter().map(|(addr, info)| (*addr, info.event_id)).collect::<Vec<_>>()
        );
        debug!(
 ...
 pub async fn polling_event(
        ...
        info!("[UNISWAP_EVENTS_POLLING] Установлен начальный блок: {}", block_from);
        ...
        loop {
            ...
            debug!(
                "[UNISWAP_EVENTS_POLLING_DEBUG] Получен block_to: {}. Количество подписанных пулов: {}", 
            ...
            while current_from <= block_to {
                ...
                match self.fetch_events(current_from, current_to).await {
                    ...
                    Err(e) => {
                        error!(
                            "[UNISWAP_EVENTS_POLLING_ERROR] Ошибка fetch_events для блоков {}–{}: {}. Количество подписанных пулов: {}", 
                            current_from, current_to, e, self.subscribed_pools.len()
                        );

                        let mut affected_pools = Vec::new();
                        for event in &pending_events {
                            if event.block_number >= current_from && event.block_number <= current_to {
                                affected_pools.push(event.address);
                            }
                        }

                        for pool_address in affected_pools {
                            let pool_event_info = PoolEventInfo {
                                event_id: self.event_counter.fetch_add(1, Ordering::SeqCst),
                                address: pool_address,
                                tick_updates: DashSet::new(),
                                current_tick: 0,
                                block_number: current_to,
                            };
                            if let Err(update_err) = self.update_graph_from_event(
                                &pool_event_info,
                                Arc::clone(&graph),
                                pool_address,
                                Arc::clone(&provider),
                                ...
                                error!(
                                    "[UNISWAP_EVENTS_POLLING_ERROR] Ошибка обновления графа для пула {:?}: {}. event_id: 🆔 {}", 
                                    pool_address, update_err, pool_event_info.event_id
                                );
                            } else {
                                debug!(
                                    "[UNISWAP_EVENTS_POLLING_DEBUG] Граф обновлен для пула {:?}. event_id: 🆔 {}", 
                                    pool_address, pool_event_info.event_id
                                );
                            }
                        }
            ...
            if !pending_events.is_empty() {
                let (event_map, changed_pools) = self.aggregate_events(pending_events.clone(), Arc::clone(&graph));

                debug!(
                    "[UNISWAP_EVENTS_POLLING_DEBUG] Собрано {} событий для {} пулов. Event_id: 🆔 {:?}", 
                    event_map.len(), changed_pools.len(), 
        ...
        let handle = tokio::spawn({
            ...
            async move {
                ...
                error!(
                    ...
                    pool_address, start_time.elapsed().as_millis()
                );
            }
        });
...
pub async fn signaler_simulator(
    mut signal_rx: mpsc::Receiver<Address>,
    signal_set: Arc<DashSet<Address>>,
    simulator_tx: mpsc::Sender<PoolEventInfo>,
) -> anyhow::Result<()> {
    let mut interval = interval(Duration::from_millis(200));
    let mut pool_addresses = HashSet::new();
    const POOL_THRESHOLD: usize = 100;

    loop {
        tokio::select! {
            // Чтение сигналов из signal_rx
            Some(pool_address) = signal_rx.recv() => {
                pool_addresses.insert(pool_address);
                debug!("[UNISWAP_SIGNALER_DEBUG] Получен сигнал для пула {:?}", pool_address);
                if pool_addresses.len() >= POOL_THRESHOLD {
                    Self::send_to_simulator(&pool_addresses, &simulator_tx).await?;
                    pool_addresses.clear();
                    signal_set.clear();
                }
            }
            // Периодическая проверка каждые 200 мс
            _ = interval.tick() => {
                // Добавить адреса из signal_set, если signal_rx закрыт
                for pool_address in signal_set.iter() {
                    pool_addresses.insert(*pool_address);
                }
                ...
                debug!("[UNISWAP_SIGNALER_DEBUG] Периодическая проверка, обработано {} пулов", pool_addresses.len());
            }
            // Обработка закрытия канала
            else => {
                error!("[UNISWAP_SIGNALER_ERROR] Канал signal_rx закрыт, завершение сигнализатора");
                // Проверить signal_set для оставшихся сигналов
                for pool_address in signal_set.iter() {
                    pool_addresses.insert(*pool_address);
                }
                if !pool_addresses.is_empty() {
    ...
    info!("[UNISWAP_SIGNALER] Сигнализатор завершил работу");
    Ok(())
}



async fn send_to_simulator(
    pool_addresses: &HashSet<Address>,
    simulator_tx: &mpsc::Sender<PoolEventInfo>,
) -> anyhow::Result<()> {
    debug!("[UNISWAP_SIGNALER_DEBUG] Отправка {} пулов в simulator_tx", pool_addresses.len());
    for &pool_address in pool_addresses {
        let event = PoolEventInfo {
            event_id: 0, // Не используется, так как симуляция по pool_address
            address: pool_address,
            tick_updates: DashSet::new(),
            current_tick: 0,
            block_number: 0,
        };
        match simulator_tx.try_send(event) {
            Ok(()) => debug!("[UNISWAP_SIGNALER_DEBUG] Пул {:?} отправлен в simulator_tx", pool_address),
            Err(e) => warn!("[UNISWAP_SIGNALER_WARN] Ошибка отправки пула {:?}: {:?}", pool_address, e),
        }
    }
    Ok(())
}
