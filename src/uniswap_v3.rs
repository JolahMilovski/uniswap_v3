...
/*
pub async fn sync_pools(graph: Arc<Mutex<UniversalGraph>>,provider: Arc<Provider<Ws>>,) -> Result<(), Box<dyn std::error::Error>> {
    // Получаем адрес фабрики из переменной окружения
    let factory_address: Address = get_env_var("UNISWAP_V3_FACTORY").parse()?;

    // Получаем начальный блок из переменной окружения
    let start_block: u64 = get_env_var("START_BLOCK").parse()?;
    info!("Начинаем синхронизацию с блока: {}", start_block);
  
    let dex = Dex::new(factory_address, DexVariant::UniswapV3, start_block, None);
   
    // Получаем все пулы
    let pools: Vec<CfmmsPool> = sync_pairs(vec![dex], provider.clone(), None).await?;
    let total_pools = pools.len();
    info!("Найдено {} пулов для синхронизации.\n", total_pools);
       
    // Разделяем пулы на батчи по 100 штук
    let batch_size = 10;
    let pool_batches: Vec<_> = pools.chunks(batch_size).collect();
    
    // Атомарная переменная для отслеживания прогресса
    let processed_pools = Arc::new(AtomicUsize::new(0));
      

    // Обрабатываем батчи параллельно
        for batch in pool_batches {
            let processed_pools_clone = Arc::clone(&processed_pools); // Клонируем для замыкания
            // Используем `iter` из `futures::stream`
            iter(batch).for_each_concurrent(10, |pool| {
                    let provider = provider.clone();
                    let graph = graph.clone();
                    let token_cache = token_cache.clone();
                    let processed_pools_clone = Arc::clone(&processed_pools_clone); // Клонируем для замыкания
                    async move {
                        ...
                        let (
                            ...
                            _ => {
                                error!("Неподдерживаемый тип пула: {:?}", pool);
                                return; // Пропускаем этот пул
                            }
                        };

                        // Проверяем существование контракта
                        let code = match provider.get_code(pool_address, None).await {
                            Ok(code) => code,
                            Err(e) => {
                                error!(
                                    "Ошибка при проверке существования контракта {:?}: {:?}\n",
                                    pool_address, e
                                );
                        ...
                        if code.is_empty() {
                            error!("Контракт пула {:?} не существует или пуст.", pool_address);
                            return;
                        }
                        let pool_contract = UniswapV3Pool::new(pool_address, provider.clone());
                       
                        let liquidity = match pool_contract.liquidity().call().await {
                            Ok(value) => U512::from(value),
                            Err(e) => {
                                error!(
                                    "Ошибка при получении liquidity для пула {:?}: {:?}\n",
                                    pool_address, e
                                );
                                U512::zero()
                            }
                        ...
                        if liquidity.is_zero() || liquidity == U512::from_dec_str("1032657129229710185650167").unwrap() || liquidity < U512::from_dec_str("1000000000000000000").unwrap(){ return;}


                        let (token_a_symbol, token_a_decimals) = match get_single_token_data(token_a, provider.clone(), &token_cache).await {
                            Ok(data) => data,
                            Err(e) => {
                                error!(
                                    "Не удалось получить данные для токена A {:?}. Пул {:?} пропущен. Ошибка: {:?}",
                                    token_a, pool_address, e
                                );
                                return; // Пропускаем весь пул
                            }
                        };

                        let (token_b_symbol, token_b_decimals) = match get_single_token_data(token_b, provider.clone(), &token_cache).await {
                            Ok(data) => data,
                            Err(e) => {
                                error!(
                                    "Не удалось получить данные для токена B {:?}. Пул {:?} пропущен. Ошибка: {:?}",
                                    token_b, pool_address, e
                                );
                                return; // Пропускаем весь пул
                            }
                        };
                        ...
                        //клонированные данные символов токена
                        let token_a_symbol_clone = token_a_symbol.clone();
                        let token_b_symbol_clone = token_b_symbol.clone();
                    
                                   
                        let (sqrt_price_x96, tick, _observation_index, _observation_cardinality, _observation_cardinality_next, _fee_protocol, _unlocked) =
                        match pool_contract.slot_0().call().await {
                            Ok((sqrt_price_x96, tick, observation_index, observation_cardinality, observation_cardinality_next, fee_protocol, unlocked)) => {
                                  

                                // Проверяем значение tick
                                if tick < -887272 || tick > 887272 {
                                    error!(
                                        "Недопустимое значение tick для пула {:?}: {}. Пул пропущен.",
                                        pool_address, tick
                                    );
                                    return; // Пропускаем этот пул
                                }
                                // Используем полученные значения, если tick в допустимом диапазоне
                                (sqrt_price_x96, tick, observation_index, observation_cardinality, observation_cardinality_next, fee_protocol, unlocked)
                            }
                            Err(e) => {
                                error!(
                                    "Ошибка при получении slot_0 для пула {:?}: {:?}. Пул пропущен.",
                                    pool_address, e
                                );
                                return; // Пропускаем этот пул
                            }
                        ...
                        // Вычисляем текущую цену с учетом decimals
                        let current_price = match calculate_current_price(
                            sqrt_price_x96.into(),
                            token_a_decimals,
                            token_b_decimals,
                        ) {
                            Ok(price) => price,
                            Err(e) => {
                                error!("Ошибка при вычислении текущей цены для пула {:?}: {:?}", pool_address, e);
                                U512::zero();
                                info!("Пул {:?} пропущен. Цена 0", pool_address);
                                return; // Пропускаем пул
                            }
                            
                        };                    

                        
                        let uniswap_tick_lower = tick - tick_spacing;
                        let uniswap_tick_upper = tick + tick_spacing;
                        
                        

                        let uniswap_max_liquidity_per_tick: U512 = match pool_contract.max_liquidity_per_tick().call().await {
                            Ok(value) => U512::from(value),
                            Err(e) => {
                                error!(
                                    "Ошибка при получении max_liquidity_per_tick для пула {:?}: {:?}\n",
                                    pool_address, e
                                );
                                U512::zero()
                            }
                        ...
                        // Получаем активные тики
                        let tick_map: HashMap<i32, (i128, U512)> = match fetch_active_ticks(pool_address, provider.clone(), tick, tick_spacing).await {
                            Ok(ticks) => {
                                if ticks.is_empty() {
                                    eprintln!("Пул {:?} пропущен: пустая тик-карта.", pool_address);
                                    return;
                                }
                        
                                let mut is_invalid = false;
                                for (_tick, (liquidity_net, sqrt_price)) in &ticks {
                                    if sqrt_price.to_string().len() > 90 {
                                        is_invalid = true;
                                        break;
                                    }
                                    if liquidity_net.unsigned_abs() > 1_000_000_000_000_000_000_000 {
                                        is_invalid = true;
                                        break;
                                    }
                                }
                                ...
                                if is_invalid {
                                    eprintln!("Пул {:?} пропущен: аномальные значения в tick_map.", pool_address);
                                    return;
                                }
                            ...
                            Err(e) => {
                                eprintln!("Ошибка получения тиков для пула {:?}: {}", pool_address, e);
                                return;
                            }
                        };
                        ...
                        graph.add_pool(
                            ...
                            token_a,
                            token_a_decimals,
                            token_a_symbol_clone,                            
                            token_b,
                            token_b_decimals,
                            token_b_symbol_clone,
                            liquidity,                                                
                            sqrt_price_x96.into(),
                            current_price,
                            tick,
                        ...
                        // Логируем прогресс
                        let current_processed = processed_pools_clone.fetch_add(1, Ordering::SeqCst);
                        if current_processed % 100 == 0 {
                            info!(
                                "Прогресс: обработано {}/{} пулов ({}%)",
                                current_processed,
                                total_pools,
                                (current_processed as f64 / total_pools as f64) * 100.0
                            );
                        }
                    }
                }
        ...
        // Сохраняем данные в файл после завершения всех батчей
        if let Err(e) = graph.lock().await.save_pool_to_file() {error!("Ошибка при сохранении пула в файл: {:?}", e);}

        info!("Синхронизация завершена. Всего пулов: {}\n", total_pools);

    println!("INFO: Синхронизация с Uniswap V3 завершена. Данные сохранены в файл.\n");
...
pub async fn fetch_active_ticks(
    ...
    let tick_lens_address: Address = env::var("UNISWAP_TICK_LENS_ARBITRUM_ADDRESS")?.parse()?;
