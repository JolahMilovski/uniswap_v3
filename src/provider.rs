...
/// Тип провайдера с поддержкой rate-limiting
#[derive(Clone)]
enum ProviderType {
    Ws(Arc<Provider<Ws>>),
...
/// Состояние провайдера с метриками и rate-limiter
struct ProviderState {
    provider: ProviderType,
    failed_attempts: AtomicU32,
    last_failed_ts: AtomicU64,
    blacklisted: AtomicBool,
   limiter: Arc<RateLimiter<NotKeyed, InMemoryState, DefaultClock>>, // Общий rate-limiter для всех запросов
}

/// Менеджер провайдеров с балансировкой нагрузки и rate-limiting
...
impl ProviderManager {
    /// Создает новый менеджер с указанным лимитом RPS (запросов в секунду)
    pub async fn new(rps_limit: u32) -> Arc<Self> {
        // Настройка квоты (например, 30 запросов/сек для Alchemy)
        let quota = Quota::per_second(NonZeroU32::new(rps_limit).unwrap());
        ...
        let mut ws_providers = Vec::with_capacity(ws_urls.len());
        let mut http_providers = Vec::with_capacity(http_urls.len());

        // Инициализация WS провайдеров
        for url in ws_urls {
            if let Ok(ws) = Ws::connect(&url).await {
                ws_providers.push(Arc::new(ProviderState {
                    provider: ProviderType::Ws(Arc::new(Provider::new(ws))),
        ...
        Arc::new(Self {
            ws_providers,
            http_providers,
            blacklist_duration_secs: 600, // 10 минут блокировки
            ws_index: AtomicUsize::new(0),
            http_index: AtomicUsize::new(0),
        })
    }

    /// Получает рабочий WS провайдер с учетом rate-limiting
    pub async fn get_ws_provider(&self) -> Option<Arc<Provider<Ws>>> {
        self.get_provider_internal(&self.ws_providers, &self.ws_index).await.map(|p| {
            if let ProviderType::Ws(provider) = p {
                ...
                unreachable!("Inconsistent provider type")
            }
        })
    }

    /// Получает рабочий HTTP провайдер с учетом rate-limiting
    ...
    /// Внутренняя логика выбора провайдера
    async fn get_provider_internal(
        ...
        // Round-robin выбор провайдера
        for _ in 0..providers.len() {
            let state = &providers[current_idx];
            current_idx = (current_idx + 1) % providers.len();

            // Проверка блокировки провайдера
            if state.blacklisted.load(Ordering::Relaxed) {
                let last_failed = state.last_failed_ts.load(Ordering::Relaxed);
                if now_secs - last_failed < self.blacklist_duration_secs {
                    continue;
                }
            ...
            let result = match &state.provider {
                ProviderType::Ws(provider) => {
                    tokio::time::sleep(Duration::from_millis(100)).await; // Защита от DDoS
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    if fails >= 3 {
                        ...
                        log::warn!("Provider blacklisted: {:?}", e);
                    }
...
/// Получает рабочий WS провайдер с бесконечными попытками
pub async fn get_working_ws_provider(rps_limit: u32) -> Arc<Provider<Ws>> {
    let manager = ProviderManager::new(rps_limit).await;
    loop {
...
/// Получает рабочий HTTP провайдер с бесконечными попытками
pub async fn get_working_http_provider(rps_limit: u32) -> Arc<Provider<Http>> {
    let manager = ProviderManager::new(rps_limit).await;
    loop {
